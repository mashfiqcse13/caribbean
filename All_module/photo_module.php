...
$query = mysqli_query($link,"SELECT p.id AS PID,p.croped AS Pstatus, u.id AS UID,u.type FROM tbl_profile_photos AS p LEFT OUTER JOIN tbl_users AS u ON u.id=p.user_id WHERE u.username='" . $_GET['username'] . "' ORDER BY p.id DESC LIMIT 3");
