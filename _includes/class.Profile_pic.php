...
class Profile_photo {
    ...
    private $size_after_auto_resize = MEMBER_IMAGE_SIZE;
    private $auto_resize = true;
    private $destination_url_after_processing = 'manage_photo.php';
    private $db;
...
//        die(print_r($file));
    ...
    function get_file_by($photo_id) {
        $db = $this->db;
        $imagename = $photo_id . ".jpg";
        $imagExtention = explode('.', $imagename);
        $file['imagExtention'] = 'jpg';

        $file['file'] = "../_uploads/profile_photo/" . $imagename;
        $file['file_url'] = BASE_URL . "_uploads/profile_photo/" . $imagename;
        $file['imagename'] = $imagename;
        $file['status'] = $result[0]['status'];
    ...
    function crop($photo_id, $cropping_data, $back_2_destinaion = TRUE) {
        $src_file_detail = $this->get_file_by($photo_id);
        $src_path = $src_file_detail['file'];
        $destinaion = str_replace('profile_photo/', 'profile_photo/croped/', $src_path);
    ...
    function uncrop($photo_id) {
        $src_file_detail = $this->get_file_by($photo_id);
        $src_path = $src_file_detail['file'];
        $croped_file = str_replace('profile_photo/', 'profile_photo/croped/', $src_path);

        $this->delete_file_by($croped_file);

        //setting image status to 33 means it is croped
        $this->update_photo_status_by($photo_id, 0);
