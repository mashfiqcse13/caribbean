...
class PHPMailer {
    ...
    protected function AddAnAddress($kind, $address, $name = '') {
        ...
        if (!self::ValidateAddress($address)) {
            $this->SetError($this->Lang('invalid_address') . ': ' . $address . "1");
            if ($this->exceptions) {
                throw new phpmailerException($this->Lang('invalid_address') . ': ' . $address . "2");
            }
            echo $this->Lang('invalid_address') . ': ' . $address . "3";
            return false;
        }
        if ($kind != 'ReplyTo') {
    ...
    public function SetFrom($address, $name = '', $auto = 1) {
        ...
        $name = trim(preg_replace('/[\r\n]+/', '', $name)); //Strip breaks and trim
        if (!self::ValidateAddress($address)) {
            $this->SetError($this->Lang('invalid_address') . ': ' . $address . "4");
            if ($this->exceptions) {
                throw new phpmailerException($this->Lang('invalid_address') . ': ' . $address);
            }
            echo $this->Lang('invalid_address') . ': ' . $address . "uh";
            return false;
        }
        $this->From = $address;
        $this->FromName = $name;
