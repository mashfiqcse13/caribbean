...
function uploadFile(
$name, $tmp_name, $error, $size, $toDir, $allowedExtensions, $maxSize, $doResize, $resizeMaxWidth, $resizeMaxHeight, $resizeQuality, $resizeOnLess, $doThumb, $thumbMaxWidth, $thumbMaxHeight, $thumbQuality
) {

    if ($error != 0) {
        $message = "There was an upload error for file  `'.$name.'`, code #" . $error . ". Check your server's configuration";
        switch ($error) {
            case UPLOAD_ERR_INI_SIZE: $message = "The uploaded file `'.$name.'` exceeds the upload_max_filesize directive in php.ini";
                break;
            case UPLOAD_ERR_FORM_SIZE: $message = "The uploaded file `'.$name.'` exceeds the MAX_FILE_SIZE directive that was specified in the HTML form";
                break;
            case UPLOAD_ERR_PARTIAL: $message = "The uploaded file  `'.$name.'` was only partially uploaded";
                break;
            case UPLOAD_ERR_NO_FILE: $message = "No file was uploaded";
                break;
            case UPLOAD_ERR_NO_TMP_DIR: $message = "Missing a temporary folder on your server";
                break;
            case UPLOAD_ERR_CANT_WRITE: $message = "Failed to write file to disk on your server";
                break;
            case UPLOAD_ERR_EXTENSION: $message = "File upload stopped by extension";
                break;
        }
    ...
    if ($doResize) {
        $err = resizeImg(
                $filePath, $resizeMaxWidth, $resizeMaxHeight, $resizeQuality, true, $resizeOnLess
        );
        if ($err != null)
    ...
    if ($doThumb) {
        $err = resizeImg(
                $filePath, $thumbMaxWidth, $thumbMaxHeight, $thumbQuality, false, true
        );
        if ($err != null)
...
function upload($doResize, $doThumb) {
    ...
    foreach ($files as $file) {
        $fileResult = uploadFile(
                $file['name'], $file['tmp_name'], $file['error'], $file['size'], $config['BaseDir'], explode(',', $rType['allowedExtensions']), $rType['maxSize'], $doResize, $config['ResizedImages']['maxWidth'], $config['ResizedImages']['maxHeight'], $config['ResizedImages']['quality'], isset($_GET['resizeOnLess']), $doThumb, $config['Thumbnails']['maxWidth'], $config['Thumbnails']['maxHeight'], $config['Thumbnails']['quality']
        );
...
function resizeImg($sourceFile, $width = 200, $height = 140, $quality = 90, $resizeself = false, $resizeOnLess = true) {
    ...
    if (!$resizeOnLess && $xscale < 1 && $yscale < 1)
        return null;
